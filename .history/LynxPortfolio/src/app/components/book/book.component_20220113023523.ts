...
export class BookComponent implements OnInit {
  ...
  async getBookImgs() {
    try {
      ...
      for(let bookImg of this.bookImgs){
        let bookImgWidthPushed: any = await this.checkImgWNH(bookImg);
        if(bookImgWidthPushed && bookImgWidthPushed.width){
          bookImg = bookImgWidthPushed;
        }
      }
      this._webService.consoleLog(
        this.bookImgs,
  ...
  async checkImgWNH(file: BookImg) {
    try {
      let img = new Image();
      img.src = file.img.location;

      await img.onload = (e: any) => {
        file.width = parseInt(e.path[0].width);
